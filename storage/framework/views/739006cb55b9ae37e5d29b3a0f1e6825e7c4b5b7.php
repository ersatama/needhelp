      ...
      function updateTimer() {
          if ($('tbody > tr').length > 0) {
              ...
              items.each(function( index ) {
                  foundItem =   $( this ).find('td').eq(8);
                  if (status) {
                      foundItem.attr('data-timer',foundItem.text().trim());
                      let date  =   new Date(parseInt(foundItem.text().trim()));
                      console.log(format_time(parseInt(foundItem.text().trim())));
                      ...
                      console.log(format_time(utcDate1.getTime()));
                      timer =   foundItem.text().trim();
                  } else {
                      timer =  foundItem.attr('data-timer');
                  }
                  foundItem.html('<span>'+date1.getMinutes()+':'+date1.getSeconds()+'</span>')
              })
              status    =   false;
/*              .each(item => {
                  foundItem =   item.find('td');
