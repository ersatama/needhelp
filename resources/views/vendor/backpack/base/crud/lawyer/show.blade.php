...
@section('content')
    ...
    @php
        ...
        $questions  =   QuestionRepositoryEloquent::_getByLawyerId($url[2]);
    @endphp
                        ...
                        <td class="fsize">
                            @if($user)
                                @if ($user->{Contract::ROLE} === Contract::LAWYER)
                                    <a href="/lawyer/{{ $user->{Contract::ID} }}/show"
                                       class="text-info"><u>{{ $user->{Contract::NAME} }} {{ $user->{Contract::SURNAME} }}</u></a>
                                @elseif ($user->{Contract::ROLE} === Contract::ADMIN)
                                    <a href="/admin/{{ $user->{Contract::ID} }}/show"
                                       class="text-info"><u>{{ $user->{Contract::NAME} }} {{ $user->{Contract::SURNAME} }}</u></a>
                                @elseif ($user->{Contract::ROLE} === Contract::USER)
                                    <a href="/user/{{ $user->{Contract::ID} }}/show"
                                       class="text-info"><u>{{ $user->{Contract::NAME} }} {{ $user->{Contract::SURNAME} }}</u></a>
                                @endif
                            @endif
                        </td>
